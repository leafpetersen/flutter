...
class SampleInfo {
  ...
  bool initialize() {
    ...
    final RegExp startRE =
        new RegExp(r'^/\*\s+^Sample\s+Catalog', multiLine: true);
    ...
    final RegExp keywordsRE =
        new RegExp(sampleCatalogKeywords, multiLine: true);
...
void generate(String commit) {
  ...
  for (String className in classToSamples.keys) {
    final Iterable<String> entries =
        classToSamples[className].map((SampleInfo sample) {
      return expandTemplate(entryTemplate, sample.commentValues);
    });
    writeExpandedTemplate(
